This project is an app launcher and menu for desktop functions such as wifi, bluetooth and volume control. The view shown on launch is a picker that lets the user fuzzy search all apps and builtin "panels".

It is written in rust and uses the gpui framework.
...
- `dbus/<module>/mod.rs` - Contains a wrapper around the dbus service and its main interface. The UI should use code from this module to interact with the service.
- `dbus/<module>/api/...` - Contains auto-generated code from `zbus-xmlgen`. Files in here should not be edited, instead wrappers should be added to the parent module. The `mod.rs` file in this module should re-export all sub-modules so they are directly accessible at `api::...`

Where it makes sense, the wrappers should directly embed properties the UI will need. As an example, a AccessPoint in the NetworkManager might look like this:

```rust
#[derive(Clone)] // Implements Clone so it can be passed between views
pub struct AccessPoint {
  pub ssid: SharedString, // SharedString can be used directly in the UI
  pub strength: u8,
  pub frequency: u32,

  // Owned proxy so we don't need lifetimes when passing this around
